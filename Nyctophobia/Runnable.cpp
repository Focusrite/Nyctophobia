...
void Runnable::setFullscreen(bool b)
{
	if( b )
	{
...
void Runnable::drawAll()
{
	...
	HR(gd3dDevice->Clear(0, 0, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_ARGB(255,0,0,0), 1.0f, 0));	// 0xffffffff
	...
	draw();
	//:TEST: Drawing the alpha
	//gGraphics->drawTexture(gGraphics->getAlphaTexture()->d3dTex(),SCREEN_WIDTH/2, SCREEN_HEIGHT/2, SCREEN_WIDTH, SCREEN_HEIGHT);
